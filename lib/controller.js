...
function addIngredient(containerId) {
    ...
    if (!weightUnitValidity.includes(false)) {
        ...
        drawTable(container, tableHeaders, ingredients);
 ...
 * @param {Array} totalValues
 */
function calculateFormula(ingredients, totalValues) {
    let ingredientsTotalWeight = 0;
    let ingredientsTotalPercentage = 0;

    console.log(totalValues);

    // Get total percentages
    let totalPercentage = ingredients.reduce((a, b) => {
        return parseFloat(a.percentage) + parseFloat(b.percentage);
    });

    if (ingredients) {
        ingredients.forEach((ingredient) => {
            ingredientsTotalPercentage += parseFloat(ingredient?.percentage);
            ingredientsTotalWeight += parseFloat(ingredient?.weightGr);
        });
    }
...
function drawTable(container, headers, ingredients) {
    utils.clearTable(container.id);
    ...
    ingredients.forEach((ingredient) => {
        utils.addRow(container, ingredient);
    });
